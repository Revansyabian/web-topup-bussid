...
async function showHistory() {
    ...
    try {
        ...
        if (!data || typeof data !== 'object') { if (list) list.innerHTML = '<p style="text-align:center;color:#666;">Belum ada transaksi</p>'; hideLoading(); return; }
        if (!currentUser || !currentUser.username) { if (list) list.innerHTML = '<p style="text-align:center;color:#666;">Session tidak valid</p>'; hideLoading(); return; }
        var arr = [];
        for (var key in data) {
            var t = data[key];
            if (t && t.type && t.operator === currentUser.username) { t.id = key; arr.push(t); }
        }
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            html += '<div class="transaction-item ' + t.type + '"><div class="transaction-header"><div>' + sanitize(t.accountName || '-') + '</div><div class="transaction-amount">' + sign + formatCurrency(t.amount || 0) + '</div></div><div class="transaction-details"><div>' + typeText + '</div><div>' + (t.timestamp ? new Date(t.timestamp).toLocaleString('id-ID') : '-') + '</div></div><div class="transaction-balance"><span>Sebelum: ' + formatCurrency(t.oldBalance || 0) + '</span><span>→</span><span>Sesudah: ' + formatCurrency(t.newBalance || 0) + '</span></div></div>';
        }
        if (list) list.innerHTML = html; hideLoading();
